...
async function main(): Promise<void> {
  ...
  try {
    ...
    console.error('[test-web-worker] Required files are missing:', error)
    process.exit(1)
  }
  ...
  if (!hasWorkerImport) {
    console.error('[test-web-worker] WebWorkerManager does not reference WebWorkerInterpreter.ts?worker as expected.')
    process.exit(1)
  }

  console.log('[test-web-worker] Worker wiring looks valid.')
}

main().catch((error) => {
  console.error('[test-web-worker] Unexpected error:', error)
  process.exit(1)
})
